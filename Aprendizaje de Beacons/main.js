...
/**
 * Escanea los beacons disponibles y los guarda en un diccionario de beacons, con su mac address como key.
 */
function startScan()
{
...
/**
 * Coloca los beacons en una lista ordenados por rssi en orden descendente
 */
function getSortedBeaconList(beacons)
{
	...
	console.log( JSON.stringify(beaconList) );
	console.log( "Inside getSortedBeaconList" );
	return beaconList;
}
...
function sendFingerprint() {
	if(window.plugins && window.plugins.WifiAdmin) {
		...
		startScan();
		var beaconList = getSortedBeaconList(beacons);
		
		
		html += "<br/>Available Beacons:<br/>";
		var timeNow = Date.now();
		while(beaconList.length > 0) {
			var item = beaconList.shift();
			//Solo añadir a la lista de envio, lo actualizados durante el scanningInterval reciente
			if (item.timeStamp + SCAN_INTERVAL > timeNow){
				html += item.address  + '(' + item.rssi + 'dB)';
				//if(item['BSSID'] === wifiConnected['BSSID']) html += '(*)';
				html += '<br/>';
				network_data.push({"mac": item.address,"rssi": item.rssi})
			}
		}
		/*
		html += "<br/>Available Beacons:<br/>";
...
function scanAndSend(results) {
	...
	if (results.buttonIndex == 1) {
		...
		sendFingerprint();
		startScan();
		scanningInterval = setInterval(sendFingerprint,SCAN_INTERVAL);
	}
	toggle = true;
}
...
function main() {
	...
	if (test == null || test.length < 1) {
		...
		servername  = '104.154.240.176:18003/';
